...
$backendDir = 'E:\Billmanagment\backend'
...
try {
  ...
  $tmpDir = Join-Path $backendDir 'tmp-scenario'
  ...
  $resultPath = Join-Path $backendDir 'scenario-result.json'
  ($result | ConvertTo-Json -Depth 10) | Set-Content $resultPath
  Write-Output ("RESULT_FILE=" + $resultPath)
  Write-Output ($result | ConvertTo-Json -Depth 10)
}
finally {
  if ($job) { Stop-Job -Id $job.Id -ErrorAction SilentlyContinue | Out-Null; Remove-Job -Id $job.Id -Force -ErrorAction SilentlyContinue | Out-Null }
}
